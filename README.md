...
- **Inbound webhooks close the loop.** Real card collection is asynchronous, so `POST /integrations/{processor}/webhook` receives the rail's signed events and advances the matching collection (paid → recovered, failed → next retry, requires-action → pause) through the *same* dunning state machine as an off-session pull. It's mounted outside `/v1` because the signature is the authentication, not an API key. Stripe is wired (set `STRIPE_WEBHOOK_SECRET`); the signature scheme is verified by tests.
...
- **Tax** — set a `tax_rate` (percent) on a customer and the engine appends a **reconcilable tax line** over the subtotal, covered by the verification hash like any other line. smolbill computes the tax line deterministically from the rate you set; it is not your tax-rate-resolution engine or your tax-of-record (wire a tax provider or a merchant-of-record rail for that).
- **Invoice** (`internal/invoice`) — deterministic invoice assembly with time-exact proration of flat fees (usage is never prorated), plus a per-line meter trace and a SHA-256 verification hash. Same inputs → same invoice → same hash. This is the basis for the reconciliation ledger. Each finalized invoice also gets a human-readable, monotonic number (`INV-000042`) from an atomic sequence — what accounting and disputes need.
